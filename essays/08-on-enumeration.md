...
*Teeth: consequences that mere similarity does not produce.*
...
- A compression-optimal physical system is found that does NOT factorize through a retained invariant — i.e., whose later states depend on full history, not just the compressed current state. This would contradict the prediction in Teeth 3.
...
- The chain identity with *On Extropy* is proved (C1): MLTT's dependency structure implements the compression operation. The formal identity is established. The physical unification (same operation on different substrates) depends on the monist argument (T2.1). The identity has teeth: three consequences (type-theoretic well-typedness as compression-optimality; the cost of state as rate-distortion penalty; compression-optimal systems exhibiting type-like discipline) that mere similarity would not produce, each with stated falsification conditions.
